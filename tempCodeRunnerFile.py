...
def generate_genome(length :int):
    return choices([0,1],k=length) #returns random 0,1 values with lenght k

def generate_population(size :int,genome_length :int):
    return [generate_genome(genome_length) for _ in size]
...
def run_evolution(populate_func:PopulateFunc,
    ...
    popultaion = populate_func()

    for i in range(generation_limit):
        popultaion = sorted(popultaion,key=lambda genome:fitness_func(genome),reverse=True)

        if printer is not None:
            printer(popultaion,i,fitness_func)
        
        if fitness_func(popultaion[0]>=fitness_limit):
            break

        next_gen = popultaion[0:2]

        for j in range(int(len(popultaion/2)-1)):
            parents = selection_func(popultaion,fitness_func)
